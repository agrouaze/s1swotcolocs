...
def run_command(command_list, step_description):
    ...
    try:
        ...
        if process.stdout: logging.debug(f"Command stdout:\n{process.stdout.strip()}")
        logging.info(f"✅ SUCCESS: {step_description} completed.")
    except subprocess.CalledProcessError as e:
        logging.error(f"❌ FAILED: {step_description} failed with exit code {e.returncode}.")
...
def stream_command(command_list, step_description):
    ...
    if return_code == 0:
        ...
        logging.error(f"❌ FAILED: The container process failed with exit code {return_code}.")
        sys.exit(1)
...
def main():
    ...
    if args.startdate and args.stopdate:
        ...
        logging.error("Invalid arguments. You must provide both --startdate and --stopdate, or neither.")
        sys.exit(1)
    ...
    docker_run_command = [
        DOCKER_BINARY_PATH,
        "run", "--rm",
        "--user", user_spec,  # <--- NEW: Add the --user flag
        "-e", "HOME=/tmp",
        "-v", f"{HOST_SOURCES_DIR}:{HOST_SOURCES_DIR}",
        "-v", f"{HOST_DATAWORK}:{HOST_DATAWORK}",
