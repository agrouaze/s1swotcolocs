...
sys.path.append('/home1/datahome/agrouaze/sources/projet_sarwave')
sys.path.append('/espace/datarmor/datahome/sources/projet_sarwave')
import paths_safe_product_family
from s1swotcolocs.utils import get_conf_content
DEFAULT_IFREMER_S1_VERSION_L1B = ['A17', 'A18', 'A21', 'A23', 'A16', 'A15']
DEFAULT_SWOT_VARIABLES = ['latitude','longitude','swh_karin'] #Level2 # ,'swh_karin_qual','sig0_karin_uncert'
...
def get_original_sar_filepath_from_metacoloc(metacolocds,cpt)->(list,dict):
    ...
    for basesafe in basesafe:
        ...
        orignal_s1 = s1ifr.get_path_from_base_safe.get_path_from_base_safe(basesafe,
                                                            archive_name='datawork')
        orignal_s1_bis = s1ifr.get_path_from_base_safe.get_path_from_base_safe(basesafe,
                                                            archive_name='scale')
...
def get_L2WAV_S1_IW_path(fullpath_s1_iw_slc,version_L1B=None):
    ...
    newdf = paths_safe_product_family.get_products_family(df,
                                    l1bversions=version_L1B)
...
def read_swot_windwave_l2_file(metacolocds,confpath)->(xr.Dataset,str):
    """
    ...
    full_path_swot = os.path.join(dir_swot_l3, date_swot_dt.strftime('%Y'), date_swot_dt.strftime('%j'), baseswotl3)
    if os.path.exists(full_path_swot):
        dir_swot_l2 = conf['SWOT_L2_AVISO_DIR']
        pattern_l2 = os.path.join(dir_swot_l2,
                                  baseswotl3.replace('Expert', 'WindWave').replace('L3', 'L2').split('_v')[0][
                                  0:-1] + '*.nc')
    ...
    return dsswotl2,pathswotl2final
...
def create_empty_coloc_res(indexes_sar_grid)->xr.Dataset:
    ...
    for vv in DEFAULT_SWOT_VARIABLES:
        for fct in fcts:
            empty_dummy_condensated_swot_coloc['%s_%s' % (vv, fct)] = xr.DataArray(np.nan,
                                                        attrs={
                                                            'description': '%s of %s variable from SWOT product' % (
                                                            fct, vv)})
    empty_dummy_condensated_swot_coloc = empty_dummy_condensated_swot_coloc.assign_coords(indexes_sar_grid)
    empty_dummy_condensated_swot_coloc = empty_dummy_condensated_swot_coloc.expand_dims(["tile_line", "tile_sample"])
    return empty_dummy_condensated_swot_coloc


def s1swot_core_tile_coloc(lontile, lattile, treeswot, radius_coloc, dsswot, indexes_sar,cpt) -> xr.Dataset:
    """
    ...
    for oneneighbor in neighbors:
        index_original_shape_swot_num_lines, index_original_shape_swot_num_pixels = np.unravel_index(oneneighbor,
                                                                                                     dsswot[
                                                                                                         'longitude'].shape)
        indices.append((index_original_shape_swot_num_lines, index_original_shape_swot_num_pixels))
            ...
            for fct in fcts:
                if np.isfinite(swotclosest[vv].values).any():
                    if np.isfinite(swotclosest[vv].values).sum() == 1:
                        ...
                        if vv == 'swk_karin':
                            maskswotswhqual = (swotclosest['swh_karin_qual'].values == 0) & (
...
def loop_on_each_sar_tiles(dssar,dsswotl2,radius_coloc,full_path_swot,
                           cpt)->(xr.Dataset,collections.defaultdict):
    """
    ...
    app_logger.info('nb NaN in the 2km SWOT grid: %i/%i', len(latswot) - maskswot.sum(),
                    len(latswot))
    ...
    for x in tqdm(range(len(all_tile_cases))):
        ...
        if np.isfinite(lontile) and np.isfinite(lattile):
            tile_swot_condensated_at_SAR_point,cpt = s1swot_core_tile_coloc(lontile,
                                                        lattile, treeswot, radius_coloc,
                                                        dsswot=dsswotl2, indexes_sar=i,cpt=cpt)
        else:
            cpt['tile_sar_with_corrupted_geolocation'] += 1
            tile_swot_condensated_at_SAR_point = create_empty_coloc_res(indexes_sar_grid=i)
    ...
    ds_colocation_grd = xr.merge(consolidated_all_tiles_colocs, )
...
def associate_sar_and_swot_seastate_params(metacolocpath,confpath,groupsar='intraburst',
                                           overwrite=True):
    """
    ...
    if 'filepath_swot' not in metacolocds:
        if 'filepath_swot' in metacolocds.attrs:
            metacolocds['filepath_swot'] = xr.DataArray(np.tile(metacolocds.attrs['filepath_swot'],
                                        len(metacolocds['coloc'])),dims='coloc')
        else:
            raise KeyError
    ...
    fullpath_iw_slc_safes,cpt = get_original_sar_filepath_from_metacoloc(metacolocds,cpt=cpt)
    ...
    for iw_slc_safe in fullpath_iw_slc_safes:
        ...
        dsswotl2,pathswotl2final = read_swot_windwave_l2_file(metacolocds, confpath=confpath)
        if dsswotl2 is not None:
            path_l2wav_sar_safe = get_L2WAV_S1_IW_path(iw_slc_safe,
                                                       version_L1B=DEFAULT_IFREMER_S1_VERSION_L1B)
            for subswath_sar in ['iw1','iw2','iw3']:
                cpt['total_suswath_sar_tested'] += 1
                fpath_out = os.path.join(outputdir, "%s" % year,
                     "%s" % month,
                     "%s" % day,
                     metacolocpath.replace('coloc_',
                    'seastate_coloc_%s_'%(os.path.basename(iw_slc_safe).replace('.SAFE',
                                                                            '-'+subswath_sar))))
                if os.path.exists(fpath_out) and overwrite is False:
                    app_logger.info('coloc file already exists: %s', fpath_out)
                else:
                    pattern_sar  = os.path.join(path_l2wav_sar_safe,'l2*'+subswath_sar+'*.nc')
                    ...
                    if len(lst_nc_sar)>0:
                        ...
                        # subset SWOT dataset
                        subswot = dsswotl2.where((dsswotl2['latitude'] >= ymin) & (dsswotl2['latitude'] <= ymax) &
                                                 (dsswotl2['longitude'] >= xmin) & (dsswotl2['longitude'] <= xmax),
                                                 drop=True)
                        lonswot = subswot["longitude"].values.ravel()
                        lonswot[lonswot > 180] += -360.0
                        points = np.column_stack((lonswot, subswot["latitude"].values.ravel()))
                        ...
                        alpha_shape_swot = alphashape.alphashape(gdfswot,
                                                                 alpha=tolerance_simplification)
                        if alpha_shape_swot.intersects(polygon_sar_swubswath):
                            l2c_ds,cpt = loop_on_each_sar_tiles(dssar, dsswotl2,
                                                            radius_coloc=conf['RADIUS_COLOC'],
                                                            full_path_swot=pathswotl2final,
                                                            cpt=cpt)

                            cpt = save_sea_state_coloc_file(colocds=l2c_ds,
                                                            fpath_out=fpath_out, cpt=cpt)
                        else:
                            app_logger.info('subswath %s SAR not intersecting this SWOT swath',
                                         subswath_sar)
                            cpt['subswath_not_intersecting_swot'] += 1
                    else:
                        app_logger.info('SAR IW Level-2 WAV product %s is not available : %s',
                                     pattern_sar)
...
def parse_args():
    ...
    parser.add_argument("--metacolocfile", required=True, help="full path of meta coloc file")
...
def main():
    ...
    associate_sar_and_swot_seastate_params(metacolocpath=args.metacolocfile, confpath=args.confpath,
