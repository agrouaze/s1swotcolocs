...
def create_combined_gdf(fseastatecolocs):
        ...
        for i in range(ds_l2c.dims['tile_line']):
            for j in range(ds_l2c.dims['tile_sample']):
                ...
                for nc_var, gdf_col in variable_mapping.items():
                    ...
                    if nc_var in ds_l2c:
                        record[gdf_col] = ds_l2c[nc_var].isel(tile_line=i, tile_sample=j).item()
                    else:
                        print(f"Warning: Variable '{nc_var}' not found in the NetCDF file. Filling with NaN.")
    ...
    mask = (np.isfinite(combined_gdf['swh_swot']) & np.isfinite(combined_gdf['hs_sar'])
            & (combined_gdf['swh_karin_std']>0) & (combined_gdf['swh_swot']>0) & (combined_gdf['swh_karin_std']<1))

    # Return the full dataframe for maps, and the filtered one for the scatter plot
    return combined_gdf, combined_gdf[mask].reset_index(drop=True), footprints_subswath_sar
...
def plot_map(ax, gdf, column, cmap, norm, title, subswath_polygons):
    ...
    if not gdf.empty:
        gdf.plot(column=column, cmap=cmap, norm=norm,
                 ax=ax, transform=ccrs.PlateCarree(),
                 edgecolor='black', linewidth=0.2)

    # Add the SWOT swath outline
    for subswath_polygon in subswath_polygons:
        ax.add_geometries([wkt.loads(subswath_polygon)], crs=ccrs.PlateCarree(),
...
def create_static_dashboard(data_files, output_filename="dashboard_statique.png"):
    ...
    print(f"--- Data loaded successfully. {len(gdf_filtered)} valid collocation points found. ---")
    ...
    plot_configs = [
        {'ax': ax_list[0], 'col': 'hs_sar', 'cmap': cmap_swh, 'norm': norm_swh, 'label': 'SWH (m)',
         'title_base': 'SAR S-1 IW VV (SWH)'},
        {'ax': ax_list[1], 'col': 'swh_swot', 'cmap': cmap_swh, 'norm': norm_swh, 'label': 'SWH (m)',
         'title_base': 'SWOT KarIn (SWH Mean)'},
        {'ax': ax_list[2], 'col': 'swh_karin_med', 'cmap': cmap_swh, 'norm': norm_swh, 'label': 'SWH (m)',
         'title_base': 'SWOT KarIn (SWH Median)'},
        {'ax': ax_list[3], 'col': 'hs_conf', 'cmap': cmap_conf, 'norm': norm_conf, 'label': 'Confidence',
         'title_base': 'SAR SWH Confidence'},
        {'ax': ax_list[4], 'col': 'swh_karin_std', 'cmap': cmap_std, 'norm': norm_std, 'label': 'Std Dev (m)',
         'title_base': 'SWOT KarIn (SWH Std)'},
        {'ax': ax_list[5], 'col': 'swh_karin_sel', 'cmap': cmap_swh, 'norm': None, 'label': 'Hs',
         'title_base': 'SWOT KarIn (selection of points after filter.)'},
        {'ax': ax_list[6], 'col': 't0m1_most_likely', 'cmap': cmap_period, 'norm': norm_period, 'label': 'Period (s)',
         'title_base': 'SAR Period (T0m1)'},
        # Corrected configuration for phs0_most_likely
        {'ax': ax_list[7], 'col': 'phs0_most_likely', 'cmap': cmap_swh, 'norm': norm_windsea_swh, 'label': 'SWH (m)',
    ...
    for config in plot_configs:
        ...
        # Plot the map
        plot_map(config['ax'], gdf4plot, config['col'], config['cmap'], norm, title, footprints_subswath_sar)

        # Add colorbar
        if norm is not None:
            fig.colorbar(ScalarMappable(norm=norm, cmap=config['cmap']), ax=config['ax'], orientation='vertical',
                         shrink=0.6, label=config['label'])
    ...
    if not x.empty and not y.empty:
        ...
        fig.colorbar(scatter, ax=scatter_ax, orientation='vertical', label='Point Density')
    else:
        # Plot an empty scatter if no data
        scatter_ax.scatter(x, y, alpha=0.6, s=15)

    # Add 1:1 line
    scatter_ax.plot([0, 8], [0, 8], 'r--', label='1:1 Line') # Assuming a fixed range for the 1:1 line
    ...
    scatter_ax.text(0.05, 0.95, stats_text, transform=scatter_ax.transAxes, fontsize=10,
