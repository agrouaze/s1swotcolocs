...
def parse_args():
    ...
    parser.add_argument(
        "--startdate", required=True, help="starting date YYYYMMDD for searching for meta colocs files and producing SWH colocs"
    )
    parser.add_argument(
        "--stopdate", required=True, help="stop date YYYYMMDD for searching for meta colocs files and producing SWH colocs"
    )
...
def main():
    ...
    logging.info('dir to search for meta colocs : %s',metadir)
    ...
    for dd in rrule.rrule(rrule.DAILY,dtstart=sta,until=sto):
        pat = os.path.join(metadir,'IW',dd.strftime('%Y'),dd.strftime('%m'),dd.strftime('%d'),'coloc_SWOT*.nc')
        lst_files += glob.glob(pat)
    logging.info('nb files meta data colocs found; %i',len(lst_files))
