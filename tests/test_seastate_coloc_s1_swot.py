...
def mock_config():
    ...
    return {
        ...
        "SWOT_L3_AVISO_FILE": os.path.join(assets_dir,"SWOT_L3_LR_SSH_Expert_026_165_20241229T165806_20241229T174933_v2.0.1.nc"),
        "SWOT_L2_AVISO_FILE": os.path.join(assets_dir,"SWOT_L2_LR_SSH_WindWave_018_555_20240729T172147_20240729T181315_PIC0_01.nc"),
        "S1_L2_WAV_FILE": os.path.join(assets_dir,'l2-s1a-iw3-wav-dv-20240729t172529-20240729t172557-054978-06b28f-e12.nc'),
        "metacoloc" : os.path.join(assets_dir,'coloc_SWOT_L3_Sentinel-1_IW_20240729T172147.nc'),
        "RADIUS_COLOC": 0.07,
    }
...
def sample_metacoloc_ds(tmp_path):
    ...
    filepath = tmp_path / "sample_metacoloc.nc"
    ds = xr.Dataset(
        {
...
def sample_sar_l2_ds():
    ...
    return xr.Dataset(
        {
            "longitude": (("tile_line", "tile_sample"), np.array([[10.0, 10.1], [10.0, 10.1]])),
            "latitude": (("tile_line", "tile_sample"), np.array([[50.0, 50.0], [50.1, 50.1]])),
        },
...
def sample_swot_l2_ds():
    ...
    return xr.Dataset(
        {
            "longitude": (("num_lines", "num_pixels"), np.array([[10.05, 10.15], [10.05, 10.15]])),
            "latitude": (("num_lines", "num_pixels"), np.array([[50.02, 50.02], [50.12, 50.12]])),
...
def test_get_original_sar_filepath_from_metacoloc(sample_metacoloc_ds):
    ...
    with patch('s1ifr.get_path_from_base_safe.get_path_from_base_safe') as mock_get_path:
        ...
        with patch('os.path.exists', return_value=True):
            paths, _ = s1_coloc.get_original_sar_filepath_from_metacoloc(metacoloc_ds, cpt)
...
def test_get_L2WAV_S1_IW_path():
    ...
    We mock the `paths_safe_product_family` library call.
    """
    mock_l1_path = "/path/to/S1A_IW_SLC.SAFE"

    # Mock the return value of the external library
    mock_dataframe = pd.DataFrame({
        'L2_WAV_E12': ["/path/to/L2_WAV.SAFE"],
        'L2_WAV_E11': [None]
    })

    with patch('paths_safe_product_family.get_products_family', return_value=mock_dataframe) as mock_get_family:
...
def test_s1swot_core_tile_coloc(sample_swot_l2_ds):
    ...
    points = np.c_[dsswot['longitude'].values.ravel(), dsswot['latitude'].values.ravel()]
    ...
    condensated_ds, _ = s1_coloc.s1swot_core_tile_coloc(lontile, lattile, treeswot, radius_coloc, dsswot, indexes_sar,
                                                        cpt)
    ...
    condensated_ds_empty, _ = s1_coloc.s1swot_core_tile_coloc(lontile_far, lattile_far, treeswot, radius_coloc, dsswot,
                                                              indexes_sar, cpt)

    assert 'nb_SWOT_points' not in condensated_ds_empty  # It should be an empty dataset from create_empty_coloc_res
...
def test_associate_sar_and_swot_seastate_params_missing_field(sample_metacoloc_ds, mock_config):
    """
